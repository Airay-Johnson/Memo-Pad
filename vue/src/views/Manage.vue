...
<div style="display: flex;">
  <div style="display:block;">
    <div style="width: 180px;height:60px;display: flex; align-items: center;border-right: black; padding-left: 20px;background-color:whitesmoke">
      <el-dropdown @command="handleDropdown">
        <div v-if="user.avatar" class="avatar-wrap">
          <img class="avatar-img" :src="user.avatar" alt="" />
        </div>
        <div v-else class="avatar-default">{{ (user.name || (user.email ? user.email.split('@')[0] : 'U'))[0].toUpperCase() }}</div>
        <template #dropdown>
          <el-dropdown-menu>
  ...
  <FloatingButton :visible="showSidebar" @toggle="showSidebar = !showSidebar" />
  <AiSidebar :visible="showSidebar" @close="showSidebar = false" />
